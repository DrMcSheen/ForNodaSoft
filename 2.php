...
class User
{
    ...
    public static function getUsers(int $ageFrom): array
    {
        ...
        // как в таблицу запихали поле from? Дальше всё криво, нужно согласовывать.
        $stmt->execute();
    ...
    {
        $stmt = self::getInstance()->prepare("SELECT id, name, lastName, from, age FROM Users WHERE name = ?");
        $stmt->execute($name);
